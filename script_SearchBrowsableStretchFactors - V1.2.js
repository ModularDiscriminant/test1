//2025/12/31 (이 문서 시작한 시각: 2025/12/31 5:47:46)
...
const lambdaUpperBoundInput = document.getElementById("lambda (upper bound) input"); //(2025/11/22 28:57:41)

const dLowerBoundInput = document.getElementById("d (lower bound) input");
const dUpperBoundInput = document.getElementById("d (upper bound) input"); //(2025/11/22 29:51:53)
...
const nPrimeUpperBoundInput = document.getElementById("n' (upper bound) input"); //(2025/11/22 29:52:10)

const gLowerBoundInput = document.getElementById("g (lower bound) input");
const gUpperBoundInput = document.getElementById("g (upper bound) input"); //(2025/11/22 29:52:23)
...
SearchButton.addEventListener("click", async function () { //(2025/11/22 10:46:51)
    const nLowerBound = parseInt(nLowerBoundInput.value); //input이 비어 있었는지 아닌지, 즉 nLowerBoundInput.value === ""였는지 아닌지는, nLowerBound가 NaN인지 아닌지로 판별할 수 있음. 즉 이렇게 입력값 검사 없이 바로 타입 변환을 해도, 아직 입력값이 valid한지 아닌지에 대한 정보도 사라지지 않고 남아 있어서 (NaN을 적절히 판별해 주기만 한다면) 괜찮을 것 같음... . ㅎㅎ (2025/11/22 21:37:52) 흠 ㅎㅎ
    ...
    //(... 아니면 parseInt, parseFloat, ... 함수 대신에 Number 함수, ... 를 써도 된..다..? ... ㅋㅋㅋㅋ (2025/11/22 29:00:44) 흠... ㅎㅎ)

    const dLowerBound = parseInt(dLowerBoundInput.value);
    const dUpperBound = parseInt(dUpperBoundInput.value); //(2025/11/22 29:53:10)
    ...
    const nPrimeUpperBound = parseInt(nPrimeUpperBoundInput.value); //(2025/11/22 29:53:32)

    const gLowerBound = parseInt(gLowerBoundInput.value);
    const gUpperBound = parseInt(gUpperBoundInput.value); //(2025/11/22 29:53:49)
    ...
    for(k1 = 0; k1 < len1; k1++)
    {
        if(!Number.isNaN(nLowerBound)) //만약 nLowerBound가 NaN이 아니라면, 즉 사용자가 정말 input으로 뭔가 입력했다면 (= 검사해야 할 조건이 존재한다면) (2025/11/22 21:44:05)
        {
            if(!(nLowerBound <= dataset[k1][0])) //근데 dataset[k1][0], 즉 데이터 dataset[k1]의 0번째 항목인 n이 nLowerBound의 이상인 상황 (조건에 부합하는 상황) 이 아니라면, continue;를 해서 반복문을 다음 턴으로 넘김 (2025/11/22 21:45:20)
            //이와 같이 코드를 짜면, 수십 개의 조건들을 체크하기 위해 if 문을 수십 개 중첩해야 하는 awkward한 상황을 피할 수 있음 (2025/11/22 21:46:03)
            {
                continue;
            }
        }
        ...
        이렇게 짜면, nLowerBound가 NaN인 경우엔 위 조건문의 결과가 항상 false가 되므로 조건문 하나로 처리가 끝나긴 함. ㅎㅎ (2025/11/22 21:50:27)
        근데, 이렇게 짰을 경우...
            ...
            · 애초에 입력값 nLowerBound가 NaN일 수 있는 시점에서... 집어넣은 조건문이 예상과 다르게 동작하면서 오류를 일으킬 가능성이 다분하고 농후하므로, (2025/11/22 21:53:59)
        저렇게 짜는 건 (코드 가독성 및 해석, 코드 유지 보수, ... 등의 측면에서) 상당히 위험할 것 같음... . ㅎㅎ
        그래서, 그냥 조건문을 두 번 중첩해서, 매우 명시적으로 nLowerBound이 NaN일 가능성을 제외하고, 그 이후에야 조건문을 (편하고 안전하게) 사용해서 원하는 조건이 충족되는지를 체크하도록 했음... . ㅎㅎ (2025/11/22 21:55:22) 흠... ㅎㅎ
        */
        if(!Number.isNaN(nUpperBound))
        {
            if(!(dataset[k1][0] <= nUpperBound)) //난 부등식의 방향이 실제 수직선 상의 방향과 일치할 때 더 직관적이라고 보기 때문에, >= 연산자 (≥) 보다는 <= 연산자 (≤) 를 쓰는 걸 좀 더 선호함... . ㅎㅎ (2025/11/22 21:57:32) 흠... ㅎㅎ
            {
                continue;
            }
        }
        ...
        HTMLTable += "<tr> ";
            ...
            HTMLTable += "<td>";
                ...
                -> 고로, 찾을 수 있는 모든 '*'를 제거하려면 정규 표현식 (regular expression) 을 활용해야 함. 구글에 'javascript remove specific characters from string'라고 검색해 보니, 'AI 개요'가 두 개의 슬래시 ('/') 안에 제거할 문자를 넣고, 그 뒤에 'global flag'인 g를 붙여서 정규 표현식을 만든 후, 그걸 replace의 첫 번째 인자로 전달하면 된다고 해서 그대로 따라해 봄..! ㅎㅎ (2025/11/22 23:17:53)
